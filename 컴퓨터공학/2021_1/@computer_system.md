...
/(root)🍖 => usr🍖 , etc🍖 , home🍖, tmp🍖, bin🍭
usr🍖 => bin🍖 , lib🍖
bin🍖 => ls🦪

이런 느낌
심벌릭 링크🍭는 lib64@ 이렇게 @가 붙고, 디랙터리🍖는 /가 붙음
...
+ usr = > user아님 unix system resource임ㅋㅋㅋ 
...
- 디렉터리는 '/' 로 구분
- 절대경로:  **/로 시작함** ( /USER1/DESKTOP/TESTFILE ) 
- 상대경로: 현 디렉터리를 기준으로 시작 ( ./TESTFILE OR TESTFILE)
...
- 현제 디렉터리 확인 : pwd
- 이동 :cd
   -ex) cd, cd /tmp, cd test
- 홈으로 이동하기
  - cd ~ , cd , cd /home/user1 (절대 경로) . cd ../home/user1 (상대경로)

- 디렉터리 내용보기 : ls
  - option : 
 ```
a : 숨길 파일을 포함하여 모든 파일의 목록을 출력함
d : 디렉터리 자체의 정보를 출력함
i : 첫 번째 행에 inode 번호를 출력함
l : 파일의 상세 정보를 출력함
A : .(마침표)와 ..(마침표 두 개)를 제외한 모든 파일 목록을 출력함
F : 파일의 종류를 표시함(*:􀀁실행 파일,􀀁/:􀀁디렉터리,􀀁@:􀀁심벌릭 링크)
L : 심벌릭 링크 파일의 경우 원본 파일의 정보를 출력함
R : 하위 디렉터리의 목록까지 출력함
...
|필드 번호|1|2|3|4|5|6|7|8|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|필드 값 |d|rwxr-xr-x|2|user1|user1|4096|11월8 23:24|공개|
|의미|파일 종류|파일접근권한- 파일 소유자-그룹 기타사용자의 r/w 권환|하드링크 개수|파일 소유자|파일이 속한 그룹|파일크기(바이트)|마지막 수정시간|파일명|
...
> ls는 윈도우의 dir, vdir과 같음

### 02 디렉터리 만들고 삭제하기

- 디렉터리 만들기 : mkdir
  - form : mkdir [option] [directorty]
  - option : 
    - -p : 하위 디렉터리 생성 시 중간 단계 디렉터리가 없으면 자동 생성함
  - ex) mkdir -p mongoose/data

- 디렉터리 지우기 : rmdir
  - form : rmdir [option] [directory]
  - option :
    - p : 위와 동일, 중간 경로 빈 디렉터리면 자동 삭제
    - 빈 디렉터리를 지우다 안 빈거 만나면 멈춤 ㅎㅅㅎ
...
파일 링크 및 검색을 할 수 있다.
```
Create , Read(Write), Update, Delete, Search, Copy, Paste,
C, R/W, U,D,S,C/P, 
...
- 파일 뒷부분 출력하기 : tail 
  - form : tail [option] [file]
  - option
    - +행번호 : 지정한 행부터 끝까지 출력
    - -숫자 : 화면에 출력할  행수를 지정 (기본값10)
    -  -f : 파일 출력을 종료하지 않고 주기적으로 계속 출력

- 파일 복사하기 : cp
  - form : cp [option] [file1(derectory)] [file2(derectory]
  - option
    - -i file2가 존재하면 엎어쓸 것인지 물어봄
    - -r 디렉터리를 복사할 때 지정함
  - ex) cp fie1 file2, cp f1 f2 f3 dir1, cp -r dir1 dir2
- 파일 이동하고 파일명 바꾸기 : mv
  - 디렉터리 이동, 디렉터리 명을 바꿀 때 사용
  - form : mv [option] [file1(derectory)] [file2(derectory]
  - option : -i 파일2가 존재하면 덮어쓸 것인지 물어 봄 
  - ex) mv file1 fil2

- 파일 삭제하기 : rm
  - form : rm [option] [file(directory)]
  - option
    - -i 파일을 정말 삭제할 지 물어 봄
    - -r 디럭터리를 삭제할 때 지정함
  - ex) rm file rm -r dir
...
- **파일 링크**란
  - 하드링크 : 기존 파일에 새로운 파일명 추가 생성(파일에 두 번쨰 이름을 부여함)
  - 심벌릭 링크 : 원본 파일을 가리키는 새로운 파일 생성(참조, 바로가기) 
...
- inode : 파일정보를 가지고 있는 구조체 (파일명, 변경시간, 소유자, 종류, 주소,, ), 외부적으로는 번호로 표시함
  - ls -i 로 확인 가능 
- 데이터 블록 : 실질 데이터의 주소를 가지고 있음, 데이터 블록 주소
...
- 빈 파일 만들기, 수정 시간 변경하기 : touch
  ...
  - option
    - -a : 접근 시간만 변경함
    - -m : 수정 시간만 변경함
    - -t : [[CC]YY]MMDDhhmm[.ss] 시간을 직접 입력 함
  - ex)
  ```
  touch test
  ls -l test
  결과
  -re-re-r-- user1 user1 0 7월 30 13:18 test
  ```
...
학습목표
```
리눅스 문서 편집기에 대해 설명할 수 있다.
vi 삽입 및 이동을 설명할 수 있다.
Vi 수정 및 삭제를 설명할 수 있다.
vi 복사 및 잘라서 붙이기를 실행할 수 있다.
```
## 리눅스 문서 편집기 
### 01 리눅스 편집기
...
- 행단위 편집기 
  - ed : 유닉스 초기 행 단위 편집기
  - ex : vi에 연결하여 사용
  - sed : 일괄 편집 기능, 스트림 편집기
- 화면 단위 편집기
  - vi : 리눅스에서 일반적으로 사용하는 화면 편집기
  - emacs : 전문가용 (GNU Emacs : 무료 배포, 별도 설치 필요함) 
- GUI 편집기
  - gedit
...
ex) 모드형은 copy, past, save 순으로 yy, p :wq지만
비 모드형은 ctrl +c , ctrl +v , ctrl +s
..
...
학습목표
```
배시 셸 환경 설정 방법을 설명할 수 있다. 
환경 설정 명령의 예를 설명할 수 있다. 
환경 설정 파일에 대해 설명할 수 있다.
셸 스크립트의 구성요소를 설명할 수 있다.
```
...
=> 스크래퍼를 만들고 실행할 때 터미널을 이용하면 정해진 시간에 자동 업데이트를 할 수 있을 거 같다.

#### 정해진 시간 한 번 실행 : at

- option
  - "-l" : 실행 대기 리스트 
  - "-r 작업번호" : atrm과 동일, 실행 대기 작업 번호 삭제
  - "-m" : 작업 완료시 user메일로 알려줌
  - "-f 파일" : 표준 입력 대신 실행할 명령을 파일로 저장함
    - at 10:10 pm
    - at 2:45 may 11
  - "-d" : 실행되기 전 작업 삭제 
...
- form : crontab [-u userID][option][fileName]
- option
  - "-e" : 사용자의 crontab 파일 편집
  - "-l" : crontab 파일 목록 출력
  - "-r: : crontab 파일 삭제

==> 이 부분은 실습이 없음,, 해보고싶은데ㅜㅜ 스크래퍼 맨들어서 해보자 ㅠㅠ
...
학습목표
```
리눅스 파일 시스템의 종류를 설명할 수 있다.
리눅스 파일 시스템의 구조를 설명할 수 있다.
파일 시스템 마운트에 대해 설명할 수 있다.
```
...
![캡처](https://user-images.githubusercontent.com/73880776/117225383-2ddb6580-ae4d-11eb-9523-dccbff8681df.GIF)
...
- 파일 시스템을 디렉터리 계층 구조의 특정 디렉터리와 연결 하는 것
...
![캡처](https://user-images.githubusercontent.com/73880776/117225809-2cf70380-ae4e-11eb-8615-ad922cfe225c.GIF)
...
- 디스크 장치의 이름
  - IDE 컨트롤러에 연결된 디스크는 /dev/hd 로 시작
  - SCSI, SATA 컨트롤러에 장착된 디스크는 /dev/sd 로 시작
  - 최근은 IDE, SCSI구분없이 **/dev/sd** 로 시용
...
```
/dev/sda : 디스크 이름
/dev/sda0 , /dev/sda1, /dev/sda2 ,,, : 디스크 파티션 이름
```

## fdisk 명령

- 디스크 파티션 생성, 삭제, 보기 등 파티션 관리
- fdisk [옵션] [장치명]
- 옵션
  - '-b' : 섹터 크기를 지정 (512,1024,2048, 4096)
  - '-l' : 파티션 테이블 출력
...
p 파티션 테이블을 출력함
b BSD 디스크 라벨을 편집함 
q 작업 내용을 저장하지 않고 종료함
c 도스 호환성을 설정함
s 새로운 빈 Sun 디스크 라벨을 생성함
d 파티션을 삭제함 
t 파티션의 시스템 ID를 변경함 (파일 시스템 종류 변경)
l 사용 가능한 파티션을 종류를 출력함
u 항목 정보를 변경·출력함
m 도움말을 출력함 v 파티션 테이블을 검사함
n 새로운 파티션을 추가함 
w 파티션 정보를 디스크에 저장하고 종료함
o 새로운 빈 DOS 파티션을 생성함
x 실린더 개수 변경 등 전문가를 위한 부가적 기능임
```

## 파티션에 파일 시스템 생성

- **파일 시스템은 이 파티션에 파일과 디렉터리를 관리하기 위한 구조임!**
- mkfs, mke2fs
...
- 리눅스 개정판 확장 파일 시스템(ext2,ext3,ext4)를 만듬
- mke2fs [옵션] [장치명]
- 옵션
```
-t 종류 파일 시스템의 종류를 지정함(기본값은 ext2)

-b 블록 크기 블록 크기를 바이트 수로 지정함

-c 배드 블록을 체크함

-f 프래그먼트 크기 프래그먼트 크기를 바이트 수로 지정함

-i inode당 바이트 수
inode당 바이트 수를 지정함(기본값은 4,096B)

-m 예약 블록 퍼센트
슈퍼 유저에게 예약해둘 블록의 퍼센트를
지정함(기본값은5%)
```
